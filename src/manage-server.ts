...
function getOperation(specs: Record<string, any>, apiPath: string, apiMethod: string): any {
  return specs?.paths?.[apiPath]?.[apiMethod.toLowerCase()];
}
...
function formatManageSuccess(data: unknown, parameters: Record<string, any> = {}) {
  return buildToolSuccessResult(String(parameters.__tool_name || 'unknown_tool'), data, {
    outputFormat: parameters.output_format,
    includeRawJson: parameters.include_raw_json,
  });
}

export function createManageServer(context: ServerContext): McpServer {
  ...
  const tools: ToolDefinition[] = [
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          query: { type: 'string', description: '可选，按模块名或描述做简单过滤' },
          output_format: {
            type: 'string',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          query: { type: 'string', description: '可选，按 API summary 或 path 过滤' },
          output_format: {
            type: 'string',
            description: '输出格式，默认 auto',
            default: 'auto',
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          api_path: { type: 'string', description: 'API 路径' },
          api_method: { type: 'string', description: 'HTTP 方法(GET, POST, PUT, DELETE等)' },
          parameters: {
            type: 'object',
  ...
  registerToolDefinitions(server, tools, {
    select_module: async (parameters) => {
      (parameters as any).__tool_name = 'select_module';
      ...
      const filteredModules = query
        ? modules.filter((m: any) => String(m.name).toLowerCase().includes(query) || String(m.description).toLowerCase().includes(query))
        : modules;
    ...
    select_api_from_module: async (parameters) => {
      (parameters as any).__tool_name = 'select_api_from_module';
      ...
      const filteredApis = normalizedQuery
        ? apis.filter((api: any) => String(api.path).toLowerCase().includes(normalizedQuery) || String(api.summary).toLowerCase().includes(normalizedQuery))
        : apis;

      return formatManageSuccess(filteredApis, parameters as Record<string, any>);
    },
    gencode_callapi: async (parameters) => {
      (parameters as any).__tool_name = 'gencode_callapi';
